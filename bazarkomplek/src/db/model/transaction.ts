...
export default class Transactions {
  ...
  static async findAll(page: number, userId: string) {
    ...
    const aggregateTotalData = [
      {
        $match: {
          $or: [{ sellerId: userIdAsObjectId }, { buyerId: userIdAsObjectId }],
        },
      },
      {
        $group: {
    ...
    const totalData = await this.collection()
      ...
      .toArray();
    const transaction = (await this.collection()
      .aggregate(aggregate)
  ...
  static async addCartTransactionByBuyerId(newTransaction: TransactionInput) {
    ...
    if (alreadyOnCart)
      throw new Error("You Have Already added this item to your Cart");
  ...
  static async deleteTransaction(transactionId: string, userId: string) {
    ...
    const existingTransaction = await this.collection().findOne({
  ...
  static async editStatusTransaction(transactionId: string, status: string) {
    const transactionObjectId = new ObjectId(transactionId);
    console.log(
      `Edit status transaction with transactionId: ${transactionObjectId}`
    );
    const existingTransaction = await this.collection().findOne({
      _id: transactionObjectId,
    });
    ...
    const editResult = await this.collection().updateOne(
      {
        _id: new ObjectId(transactionObjectId),
      },
      {
